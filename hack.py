...
client = TelegramClient('Mr_Agora', api_id, api_hash).start(bot_token=token)
...
async def userinfo(strses):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    k = await X.get_me()
    try:
      await X(join("@Team_agora"))
    except BaseException:
      pass
    try:
      await X(join("@Agora_robots"))
    except BaseException:
      pass
    try:
...
async def user2fa(strses):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    ...
    try:
      ...
      pass
    try:
      await X.edit_2fa('MR_AGORA IS BEST')
      return True
    except:
      return False
...
async def op(event):
  ...
  if not event.is_private:
    ...
    await event.reply("Click Below To Use Me", buttons=Mr_Agora)
  else:
    legendbye = [
      [
        Button.url("Must Join", f"https://t.me/Agora_robots")
        ]
      ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
    ...
    await event.reply("Banning all members. Thanks For using Mr_Agora Bot", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\nThanks For using Mr_Agora Bot", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
    ...
    await event.reply("Joined the Channel/Group Thanks For using Mr_Agora Bot", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("Deleted the Channel/Group Thanks For using Mr_AgoraBot.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i:
        await event.reply("User don't have two step thats why now two step is `Mr_Agora Bot Is best` you can login now\n\nThanks For using Mr_Agora Bot.", buttons=keyboard)
      else:
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(f"Done Gcasted In {i} Private😗😗\n\nThanks For Using Agora_ssgenBot.", buttons=keyboard)
